...
exports.changePassword = async (req, res) => {
   try{     
    ...
    const {oldPassword,newpassword,confirmpassword} = req
    ...
    //validation
        if (!newpassword || !confirmpassword && await bcrypt.compare(password, user.newpassword)){
          return res.status(400).json({
               success:false,
    ...
    //update pwd in DB
    const newpassword = await updatepassword.create({
          updatepassword,
    })
    ...
    const mailSender = async (password,updatepassword) => {
        try{
         let transporter = nodemailer.updatepassword({
         ...
         console.log("your are password is change",info);
         return info;
    ...
    //return response
       ...
       return res.status().json({
          success:flase,
          message:"error for updated password"
       })
   }
}
