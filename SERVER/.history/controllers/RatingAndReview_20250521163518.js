...
if(!alreadyreview){
   return res.status(400).json({
    ...
    updatecourse = await Course.findByIdAndUpdate({_id:courseId},
                                                {
                                                  $push:{
...
//getAveargeRating
exporsts.getAveargeRating = async(req,res)=>{
    try{
       //get course Id
       ...
       if(result.length > 0){  
         ...
         //if no rating/Raview exist
         return res.status(200).json({
            success:true,
            message:'Avearge Rating is 0, no rating given till now',
            averageRating:0,
         })
    }catch(error){
       console.log(error);
...
exports.getAllRating = async(req,res) => {
     try{
         const allReviews = await RatingAndRe
     }catch(err){
